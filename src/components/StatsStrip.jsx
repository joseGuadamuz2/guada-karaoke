...
export default function StatsStrip() {
  return (
    <div className="relative z-10 bg-brand-dark-2 border-t border-b border-white/5 py-6 shadow-sm">
      <div className="max-w-5xl mx-auto px-6 flex flex-wrap items-center justify-center gap-8 md:gap-16">
        {STATS.map((stat, i) => (
          <div key={i} className="flex items-center gap-8">
            ...
            {i < STATS.length - 1 && <div className="hidden sm:block w-px h-12 bg-white/5" />}
          </div>
        ))}
      </div>
    </div>
