...
/**
 * Pill de característica del servicio.
 *
 * @param {object} props
 * @param {string} props.iconName - Nombre del ícono en ICON_MAP
 * @param {string} props.text     - Texto descriptivo
 */
function FeaturePill({ iconName, text }) {
  const Icon = ICON_MAP[iconName] ?? Zap;
  return (
    <div className="flex items-center gap-2 bg-white/3 border border-white/8 px-4 py-2">
      <Icon size={14} className="text-brand-red" />
      <span className="font-rajdhani text-xs tracking-[1.5px] uppercase text-gray-500">
        {text}
      </span>
    </div>
  );
...
/**
 * Sección final de llamada a la acción con botón de WhatsApp.
 */
  ...
  return (
    <div className="relative py-32 px-6 text-center bg-brand-dark overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                      w-[600px] h-[300px] pointer-events-none
                      bg-[radial-gradient(ellipse,rgba(192,57,43,0.12)_0%,transparent_70%)]" />
      ...
      >
        ...
        {/* Título */}
        <h2 className="font-bebas text-[clamp(3rem,7vw,5.5rem)] tracking-wide leading-none max-w-2xl mx-auto mb-4">
          Reserva tu <span className="text-brand-red">Noche Perfecta</span> Hoy
        </h2>
        <p className="font-inter text-gray-600 mb-10 text-lg">
        ...
        <a
          ...
          className="inline-flex items-center gap-3 bg-gradient-to-r from-[#25D366] to-[#128C7E]
                     text-white font-rajdhani font-bold text-lg tracking-[2px] uppercase px-10 py-5
                     transition-all duration-300 hover:-translate-y-1
                     hover:shadow-[0_10px_40px_rgba(37,211,102,0.3)]
