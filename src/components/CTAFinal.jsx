...
function FeaturePill({ iconName, text }) {
  ...
  return (
    <div className="flex items-center gap-2 bg-white border border-black/10 px-4 py-2 shadow-sm">
      <Icon size={14} className="text-brand-red" />
      <span className="font-rajdhani text-xs tracking-[1.5px] uppercase text-gray-500">{text}</span>
    </div>
  );
}
...
export default function CTAFinal() {
  ...
  return (
    <div className="relative py-32 px-6 text-center bg-brand-dark overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] pointer-events-none bg-[radial-gradient(ellipse,rgba(192,57,43,0.07)_0%,transparent_70%)]" />
      <div ref={ref} className={`relative z-10 transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <h2 className="font-bebas text-[clamp(3rem,7vw,5.5rem)] tracking-wide leading-none max-w-2xl mx-auto mb-4 text-gray-900">
          Reserva tu <span className="text-brand-red">Noche Perfecta</span> Hoy
        </h2>
        <p className="font-inter text-gray-500 mb-10 text-lg">Respuesta garantizada en menos de 2 horas. Sin compromiso.</p>
