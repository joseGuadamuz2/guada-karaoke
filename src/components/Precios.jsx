...
function PaqueteCard({ name, price, hours, highlight, tag, includes, cta }) {
  ...
  return (
    <div className={`relative flex flex-col p-8 border transition-all duration-300
      ${highlight
        ? 'border-brand-red bg-brand-red/10 shadow-[0_4px_30px_rgba(230,0,0,0.2)] scale-105'
        : 'border-white/10 bg-brand-dark-2 hover:border-brand-red/40 hover:shadow-md'
      }`}>
      {tag && (
      ...
      <div className="font-bebas text-5xl text-white tracking-wide leading-none mb-1">{price}</div>
      ...
      <div className={`w-full h-px mb-6 ${highlight ? 'bg-brand-red/30' : 'bg-white/10'}`} />
      <ul className="flex flex-col gap-3 flex-1 mb-8">
        {includes.map((item, i) => (
          <li key={i} className="flex items-start gap-3">
            <Check size={15} className={`mt-0.5 flex-shrink-0 ${highlight ? 'text-brand-red' : 'text-gray-400'}`} />
            <span className="font-inter text-sm text-gray-300 leading-snug">{item}</span>
          </li>
        ))}
      </ul>
      <a href={waUrl} target="_blank" rel="noreferrer"
        className={`flex items-center justify-center gap-2 font-rajdhani font-bold text-sm
                    ...
                    ${highlight
                      ? 'bg-brand-red hover:bg-brand-red-light text-white hover:shadow-[0_4px_20px_rgba(230,0,0,0.35)]'
    ...
    <section id="precios" className="py-24 px-6 bg-brand-dark-3">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-14">
          ...
          <h2 className="font-bebas text-[clamp(2.5rem,5vw,4rem)] tracking-wide leading-none mb-2 text-white">
        ...
        <div className="flex flex-col items-center gap-3 mt-10">
          <div className="inline-flex items-center gap-3 bg-brand-dark border border-white/10 px-6 py-3 shadow-sm">
            <Zap size={14} className="text-brand-red" />
            <span className="font-rajdhani text-sm tracking-[1.5px] uppercase text-gray-400">
              Hora extra: <strong className="text-white">{HORA_EXTRA}</strong>
              <span className="text-gray-500 ml-1 normal-case tracking-normal font-inter text-xs">+ IVA</span>
            </span>
          </div>
