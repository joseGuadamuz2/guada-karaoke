...
function ServicioCard({ iconName, name, desc, tags }) {
  ...
  return (
    <div className="group relative bg-white p-8 border border-black/8 overflow-hidden transition-all duration-300 cursor-default hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:border-brand-red/30">
      ...
      <h3 className="font-bebas text-2xl tracking-wide mb-3 text-gray-900">{name}</h3>
      <p className="font-inter text-gray-500 text-sm leading-relaxed mb-6">{desc}</p>
      <div className="flex flex-wrap gap-2">
        {tags.map((tag, i) => (
          <span key={i} className="font-rajdhani text-[10px] tracking-[1.5px] uppercase px-2.5 py-1 border border-black/10 text-gray-400 bg-gray-50">{tag}</span>
        ))}
      </div>
    </div>
  );
